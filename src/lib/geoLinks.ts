/**
 * Geographic proximity linking engine for Alpes-Maritimes (06).
...
export function getNearbyCommunes(
  ...
  return allCommunes
    ...
    .map(c => {
      ...
      // Approximate physical distance in km using local projection (43.70°N for Nice)
      // 1 degree of Latitude ≈ 111.1 km
      // 1 degree of Longitude ≈ 111.1 * cos(43.70°) ≈ 80.3 km
      const dLat = (lat - currentLat) * 111.1;
      const dLon = (lon - currentLon) * 80.3;
