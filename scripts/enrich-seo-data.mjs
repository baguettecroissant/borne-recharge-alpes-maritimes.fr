 ...
 * - distanceNice (distance to Nice center)
...
// Nice center coordinates
const NICE_LAT = 43.700936;
const NICE_LON = 7.268391;
...
function getProfilCommune(pop) {
  if (pop >= 300000) return 'métropole régionale';
  if (pop >= 35000) return 'grande ville';
...
for (const c of communes) {
  ...
  if (c.latitude && c.longitude) {
    c.distanceNice = Math.round(haversineKm(c.latitude, c.longitude, NICE_LAT, NICE_LON) * 10) / 10;
  } else {
    c.distanceNice = null;
  }
  // Delete the old Bordeaux compatibility key
  delete c.distanceBordeaux;
...
console.log(`✅ Enriched ${enriched} communes with Alpes-Maritimes SEO data fields.`);

// Print sample
const sample = communes[0];
console.log('\nSample (Nice):');
console.log(`  distanceNice: ${sample.distanceNice} km`);
