#!/usr/bin/env node
/**
 * Verify content uniqueness across all local pages in Alpes-Maritimes.
...
for (const category of CATEGORIES) {
  ...
  for (const c of communes) {
    const combo = [
      ...
      c.distanceNice <= 5 ? 'core' : c.distanceNice <= 15 ? 'inner' : c.distanceNice <= 35 ? 'mid' : 'outer',
...
for (const c of communes) {
  ...
  distanceSet.add(c.distanceNice);
