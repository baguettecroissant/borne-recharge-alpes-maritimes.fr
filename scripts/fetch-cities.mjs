...
async function fetchCities() {
  const url = 'https://geo.api.gouv.fr/departements/06/communes?fields=nom,code,population,codesPostaux,centre';
  
  console.log('Fetching Alpes-Maritimes (06) communes from Geo API...');
  try {
    ...
    // Process and filter communes in Alpes-Maritimes (06) with population >= 2000
    const processed = data
      .map(c => {
        let lat = 43.700936; // Nice lat
        let lng = 7.268391;  // Nice lng
